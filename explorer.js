...
function toggleGDP(on){
  ...
  fetch('https://api.worldbank.org/v2/country/all/indicator/NY.GDP.PCAP.CD?format=json&date=2022')
    .then(r=>r.json()).then(arr=>{
      ...
      map.addLayer({
        id:'gdp', type:'fill', source:'borders',
        paint:{
          'fill-color':['match',['get','ISO'], ...stops.flat(), '#cccccc'],
          'fill-opacity':0.65
        }
      }, 'borders');
    });
...
function toggleQuakes(on){
  ...
  if (map.getLayer('quakes')) {
    map.removeLayer('quakes'); map.removeSource('quakes');
  }
  if (!on) return;
  loadQuakes();
...
function toggleFires(on){
  ...
  if (map.getLayer('fires')) {
    map.removeLayer('fires'); map.removeSource('fires');
  }
  if (!on) return;
  loadFires();
...
async function loadFires(){
  try {
    const js = await fetch('https://eonet.gsfc.nasa.gov/api/v3/events?category=wildfires&status=open&days=7')
      .then(r=>r.json());
    const feats = [];
    js.events.forEach(ev =>
      ev.geometry.forEach(g => {
        if (g.type==='Point') feats.push({
          type:'Feature',
...
function toggleNight(on){
  if (map.getLayer('night')) {
    map.removeLayer('night'); map.removeSource('night');
  }
  if (!on) return;
  map.addSource('night',{
    type:'raster',
    tiles:['https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/VIIRS_SNPP_DayNightBand_At_Sensor_Radiance_Quick_Look/GoogleMapsCompatible_Level8/{z}/{x}/{y}.jpg'],
    tileSize:256,
    attribution:'NASA GIBS VIIRS DNB'
  });
  map.addLayer({ id:'night', type:'raster', source:'night', paint:{ 'raster-opacity':0.6 }});
}
